...
func runCommandSequence(ctx context.Context, cmdCh chan<- string, readyCh <-chan struct{}, marker string) {
	...
	if runtime.GOOS == "windows" {
		loadingCmd = "Write-Host 'Loading...'; for ($i=0; $i -le 25; $i++) { $p = '#' * $i; $r = ' ' * (25 - $i); $pct = $i * 4; Write-Host -NoNewline \"[$p$r] - $pct%`r\"; Start-Sleep -Milliseconds 10 }; echo ''"
	} else {
		loadingCmd = "echo 'Loading...'; i=0; while [ $i -le 25 ]; do printf '['; j=0; while [ $j -lt $i ]; do printf '#'; j=$((j+1)); done; j=0; while [ $j -lt $((25-i)) ]; do printf ' '; j=$((j+1)); done; printf '] - %s%%' $((i*4)); printf '\r'; sleep 0.01; i=$((i+1)); done; echo ''"
	}
